...
* Rendering
  ...
  Enable the Solarized "light" theme ("dark" is equally good):
  #+begin_src emacs-lisp
...
** Shell
   ...
   on matches to the current input. We also bind a helm search.
   #+begin_src emacs-lisp
     (define-key comint-mode-map [(backtab)]
       'comint-previous-matching-input-from-input)
     (define-key comint-mode-map (kbd "M-i") 'helm-comint-input-ring)
   #+end_src
...
** Org-mode
   ...
   Integrate org-mode with reftex for easily adding bibtex references:
   #+begin_src emacs-lisp
...
*** TODO Use org-dotemacs to implement this file
...
** Capture
   ...
   for later processing. The =org-capture= module implements this, and
   we define a global keybinding that lets us capture arbitrary
   information from anywhere:
   #+begin_src emacs-lisp
     (define-key global-map [(super c)] 'org-capture)
   #+end_src
