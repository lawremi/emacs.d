...
** Web
   We bind a key to use the built-in Emacs web browser (EWW) to search
   for a selected word:
   #+begin_src emacs-lisp
   (global-set-key (kbd "s-g") 'eww-search-words)
   #+end_src
...
** Version control
   ...
   #+begin_src emacs-lisp
   (define-key global-map [(super u)] 'github-search-clone-repo)
   #+end_src
